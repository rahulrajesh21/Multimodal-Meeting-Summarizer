...
const Testimonial = ({ quote, author, role }: { quote: string, author: string, role: string }) => (
  <div className="border border-white/10 p-8 flex flex-col justify-between bg-white/[0.01]">
    <div className="text-white/20 text-4xl font-serif leading-none mb-4">"</div>
    <p className="text-[13px] text-white/70 font-mono leading-relaxed mb-8">{quote}</p>
    <div className="flex flex-col">
      <span className="font-mono font-bold text-sm text-white/90">{author}</span>
      <span className="font-mono text-[11px] text-white/40">{role}</span>
    </div>
  </div>
);

const PricingCard = ({ tier, price, features, isHighlighted = false }: { tier: string, price: string, features: string[], isHighlighted?: boolean }) => (
  <div className={`border p-8 flex flex-col bg-[#0a0a0a] relative overflow-hidden transition-all duration-300 ${isHighlighted ? 'border-white/40 shadow-[0_0_30px_rgba(255,255,255,0.05)]' : 'border-white/10 hover:border-white/20'}`}>
    {isHighlighted && <div className="absolute top-0 left-0 w-full h-1 bg-white/40" />}
    <h4 className="text-lg font-mono text-white/60 mb-2 uppercase tracking-wider">{tier}</h4>
    <div className="text-4xl font-mono font-bold mb-8">{price}<span className="text-lg text-white/40 font-normal">{price !== 'Custom' ? '/mo' : ''}</span></div>
    ...
    <button className={`w-full py-3 rounded-sm font-mono text-[12px] font-bold transition-colors ${isHighlighted ? 'bg-white text-black hover:bg-gray-200' : 'bg-[#111] text-white hover:bg-[#222] border border-white/10'}`}>
  ...
  return (
    <div className="relative min-h-screen bg-[#080808] text-white font-mono overflow-x-clip selection:bg-white/30 selection:text-white">
      ...
      <div className="fixed inset-0 pointer-events-none flex justify-center z-0">
        <div className="w-[85vw] h-full border-x border-white/10 relative">
          <div className="absolute top-[80px] w-full border-t border-white/10" />
          <div className="absolute top-[70vh] w-full border-t border-white/10" />
          <div className="absolute bottom-[280px] w-full border-t border-white/10" />
          
          {/* Crosshairs */}
          <div className="absolute top-[70vh] left-0 -translate-x-1/2 -translate-y-1/2 text-white/40 text-[10px] select-none">+</div>
          <div className="absolute top-[70vh] right-0 translate-x-1/2 -translate-y-1/2 text-white/40 text-[10px] select-none">+</div>
          
          <div className="absolute bottom-[280px] left-0 -translate-x-1/2 -translate-y-1/2 text-white/40 text-[10px] select-none">+</div>
          <div className="absolute bottom-[280px] right-0 translate-x-1/2 -translate-y-1/2 text-white/40 text-[10px] select-none">+</div>
        </div>
      </div>
      ...
      <div className="relative z-10 max-w-[85vw] mx-auto flex flex-col min-h-screen">
        ...
        <nav className="flex items-center justify-between h-[80px] flex-shrink-0">
          <div className="font-sans font-medium tracking-tight text-lg flex items-center gap-2">
             <div className="grid grid-cols-3 gap-[2px]">
               <div className="w-[4px] h-[4px] bg-white rounded-[1px]" />
               <div className="w-[4px] h-[4px] bg-transparent rounded-[1px]" />
               <div className="w-[4px] h-[4px] bg-white rounded-[1px]" />
               <div className="w-[4px] h-[4px] bg-white rounded-[1px]" />
               <div className="w-[4px] h-[4px] bg-white rounded-[1px]" />
               <div className="w-[4px] h-[4px] bg-white rounded-[1px]" />
          ...
          <div className="hidden md:flex items-center gap-10 text-[11px] text-white/40 tracking-widest font-mono">
            {['Home', 'Features', 'Benefits', 'Pricing', 'About'].map(item => (
          ...
          <button className="flex items-center justify-center bg-white text-black px-4 py-1.5 rounded-md text-[11px] font-mono font-bold hover:bg-gray-200 transition-colors">
            Get Started
          </button>
        </nav>
        ...
        <div className="relative w-full flex-grow flex flex-col justify-end pb-16 pt-[20vh]">
          ...
          <div className="flex flex-col md:flex-row justify-between items-end gap-10">
            ...
            <div className="max-w-[400px] flex flex-col items-start md:items-end text-left md:text-right">
              ...
              <button className="flex items-center justify-center bg-white text-black px-6 py-2.5 rounded-full text-[13px] font-mono font-bold hover:bg-gray-200 transition-colors min-w-[140px]">
                Join Waitlist
              </button>
            </div>
          </div>
        ...
        {/* Logos Section */}
        <div className="py-16 relative border-t border-white/10 flex flex-wrap justify-center gap-6 px-4">
           {[1, 2, 3, 4, 5].map((i) => (
             <div key={i} className="border border-dashed border-white/20 bg-white/[0.02] px-8 py-4 flex items-center justify-center opacity-50 grayscale hover:opacity-100 transition-opacity min-w-[160px]">
                <div className="font-mono text-sm font-bold tracking-widest flex items-center gap-2">
                  <div className="w-4 h-4 border border-current rounded-sm" /> PARTNER {i}
                </div>
             </div>
           ))}
        </div>
        ...
        <div className="py-24 grid grid-cols-1 md:grid-cols-3 gap-6 relative border-t border-white/10">
           <Testimonial quote="This tool completely changed how our engineering team operates. We save hours every week." author="Jane Doe" role="CTO, TechCorp" />
           <Testimonial quote="The accuracy of the action items is uncanny. It's like having a dedicated chief of staff." author="John Smith" role="VP Product" />
           <Testimonial quote="Finally, a meeting assistant that respects privacy and actually delivers actionable insights." author="Alice Chen" role="Founder" />
        </div>

        {/* Pricing */}
        <div className="py-32 relative border-t border-white/10">
           <h3 className="text-3xl font-mono font-bold mb-16 text-center">Transparent Pricing</h3>
           <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
             <PricingCard tier="Starter" price="$0" features={["10 Meetings/mo", "Basic Summaries", "7-Day History"]} />
             <PricingCard tier="Pro" price="$29" features={["Unlimited Meetings", "Proactive Insights", "Unlimited History", "Custom Vocabulary"]} isHighlighted />
             <PricingCard tier="Enterprise" price="Custom" features={["Dedicated Infrastructure", "SSO & SCIM", "Custom Integrations", "24/7 Support"]} />
           </div>
        </div>
        ...
        <div className="relative border border-white/10 overflow-hidden" style={{ background: '#000', minHeight: '320px' }}>
          ...
          <div className="relative z-[2] flex flex-col items-center justify-center text-center px-6 py-20">
            ...
            <button className="flex items-center gap-3 bg-white text-black px-5 py-2.5 rounded-md font-mono font-bold hover:bg-gray-100 transition-colors" style={{ fontSize: '13px' }}>
              Get Started
              <div className="w-5 h-5 bg-black rounded-[3px] flex items-center justify-center flex-shrink-0">
                <span className="text-white text-[10px] font-mono leading-none font-bold">↵</span>
              </div>
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-[200px_1fr_1fr_1fr_1fr] gap-12 md:gap-8 mb-14">
            ...
            <div className="flex flex-col gap-4">
              <div className="flex items-center gap-2 text-white/90 text-sm font-bold">
                <div className="grid grid-cols-3 gap-[2px]">
                  {[1,0,1,1,1,1,1,0,1].map((on, i) => (
                    <div key={i} className="w-[4px] h-[4px] rounded-[1px]" style={{ background: on ? 'white' : 'transparent' }} />
                  ))}
                </div>
                Modelence
              </div>
              ...
              <p className="text-white/20 text-[10px] mt-2">© 2026 Modelence Inc.</p>
            </div>
