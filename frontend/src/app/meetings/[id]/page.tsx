        ...
        <div style={{ minHeight: '100vh', background: PAGE_BG, fontFamily: 'system-ui, -apple-system, sans-serif' }}>
            <div style={{ maxWidth: 1500, margin: '0 auto', padding: '28px 32px' }}>
                ...
                <div style={{ minWidth: 0, marginBottom: 24 }}>
                    ...
                    {/* Breadcrumb */}
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: TEXT_MUTED, marginBottom: 12 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'minmax(500px, 5fr) minmax(320px, 2fr)', gap: 32 }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                        ...
                        <div style={{ borderRadius: 12, overflow: 'hidden', marginBottom: 20, border: `0.5px solid ${BORDER}` }}>
                            {job.status === 'processing' || job.status === 'queued' ? (
                                ...
                                <video ref={videoRef} controls style={{ width: '100%', maxHeight: 320, display: 'block', background: '#000' }} src={videoSrc} />
                            ) : (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', height: 'max-content' }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                            ...
                            <button style={{ fontSize: 18, color: TEXT_MUTED, background: 'none', border: 'none', cursor: 'pointer' }}>⋮</button>
                        </div>
                        {decisions.length > 0 ? (
                            <div style={{ background: '#F8F5FF', borderRadius: 12, padding: '18px 20px 24px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: BRAND, marginBottom: 20, fontWeight: 500 }}>
                                    <Sparkles style={{ width: 14, height: 14 }} /> Generated by AI
                                </div>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                                    {decisions.slice(0, 4).map((ev: any, i: number) => (
                                        <div key={i} style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                                            <div style={{ width: 18, height: 18, border: '1.5px solid #D1D5DB', borderRadius: 4, marginTop: 2, flexShrink: 0, background: '#fff' }} />
                                            <div>
                                                <div style={{ fontSize: 14, color: TEXT_PRIMARY, lineHeight: 1.5 }}>{(ev.summary || '').slice(0, 80)}</div>
                                                <div style={{ fontSize: 12, color: BRAND, marginTop: 6, display: 'flex', alignItems: 'center', gap: 6, fontWeight: 500 }}>
                                                    <span>@{Math.floor((ev.start_time || 0) / 60)}:{(Math.floor(ev.start_time || 0) % 60).toString().padStart(2, '0')}</span>
                                                    <span style={{ color: TEXT_MUTED, fontWeight: 400 }}>·</span>
                                                    <Users style={{ width: 12, height: 12, color: TEXT_MUTED }} /> <span style={{ color: TEXT_MUTED, fontWeight: 400 }}>{ev.speaker}</span>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                            ...
                            <div style={{ fontSize: 14, color: TEXT_MUTED }}>No actionable tasks extracted.</div>
                        )}
                        ...
                        {speakerStats.length > 0 && (
                            <>
                                ...
                                <div style={{ marginBottom: 24 }}>
                                    <div style={{ ...rightSecHeader, marginBottom: 20 }}>Talk Time</div>
                                            ...
                                            return (
                                                <div key={name}>
                                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                                                        <span style={{ fontSize: 14, color: TEXT_PRIMARY }}>{name}</span>
                                                        <span style={{ fontSize: 13, color: TEXT_MUTED }}>{fmtTime(secs)} · {pct}%</span>
                                                    </div>
                                                    <div style={{ height: 6, background: PAGE_BG, borderRadius: 3 }}>
                        ...
                        {topics.length > 0 && (
                            <>
                                ...
                                <div style={{ marginBottom: 12 }}>
                                    <div style={{ ...rightSecHeader, marginBottom: 16 }}>Topics</div>
                                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                                        {topics.map(t => {
                                            const ts = topicStyle(t);
                                            return <span key={t} onClick={() => { setTopicFilter(topicFilter === t ? 'all' : t); setActiveTab('transcript'); }} style={{ fontSize: 13, fontWeight: 500, padding: '6px 14px', borderRadius: 100, background: ts.bg, color: ts.color, cursor: 'pointer' }}>{t}</span>;
                                        })}
                                    </div>
                                </div>
                            </>
