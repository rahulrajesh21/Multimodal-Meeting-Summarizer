...
function MeetingCard({ job, index }: { job: any; index: number }) {
  ...
  // Pull first real decision from API (pre-computed in list endpoint)
  const firstDecision: string | null = job.first_decision || null;

  // Real date from job
  const jobDate = job.completed_at || job.created_at;
  const dateLabel = jobDate
    ? format(new Date(jobDate), 'MMM dd, yyyy')
    : '—';

  // Real participants
  const participants: { name: string; role: string }[] = job.participants || [];
  const participantCount = participants.length || 0;

  const avatarColors = ['#4F46E5', '#16A34A', '#D97706', '#DC2626', '#0891B2'];
    ...
    <motion.div
      ...
      <div style={{
        ...
        <div style={{
          ...
          {job.scored_count ? `${job.scored_count} segs` : job.status === 'done' ? 'Done' : job.status}
        </div>
      </div>
      ...
      <div style={{ padding: '16px 18px' }}>
        ...
        <div style={{
          ...
          {dateLabel} · {participantCount} participant{participantCount !== 1 ? 's' : ''}
        </div>

        {/* Key Decision — only shown if one exists */}
        {firstDecision && (
          <div style={{
            ...
            <p style={{ fontSize: '12px', lineHeight: 1.5, color: inkSec, margin: 0 }}>
              {firstDecision.length > 120 ? firstDecision.slice(0, 117) + '…' : firstDecision}
            </p>
          </div>
        )}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex' }}>
            {(participants.length > 0 ? participants.slice(0, 4) : []).map((p, i) => (
              <div key={i} style={{
            ...
            {participants.length > 4 && (
              <div style={{
                ...
                border: '2px solid #FFFFFF', display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: '9px', fontWeight: 700, color: '#fff',
                background: '#9CA3AF', marginLeft: '-6px',
...
export default function DashboardPage() {
  ...
  const [feedItems, setFeedItems] = useState<{ ts: string; msg: string; job_id: string; title: string }[]>([]);
  const router = useRouter();
  ...
  const refreshFeed = useCallback(async () => {
    try {
      const res = await fetch(`${API}/api/feed?limit=8`);
  ...
  useEffect(() => {
    ...
    const feedId = setInterval(refreshFeed, 10_000);
  ...
  const displayCards = done.slice(0, 3);
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '36px' }}>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px' }}>
              <StatCard label="Events Tracked" value={isLoading ? '—' : totalEvents} color={indigo} icon={Zap} change="25.3%" delay={0} hero />
              <StatCard label="Total Meetings" value={isLoading ? '—' : jobs.length} color={indigo} icon={Video} change="6.4%" delay={0.08} />
              <StatCard label="In Queue" value={isLoading ? '—' : queue.length} color={amber} icon={Activity} change="10.5%" delay={0.16} />
              <StatCard label="Processed" value={isLoading ? '—' : done.length} color={positive} icon={CheckCircle2} change="4.6%" delay={0.24} />
            </div>
          </div>
        ...
        <div style={{ width: '280px', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: '20px', overflowY: 'auto' }}>
          ...
          <motion.div initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }}
            ...
            <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {feedItems.length === 0 ? (
                <span style={{ fontFamily: '"JetBrains Mono", monospace', fontSize: '11px', color: inkFaint }}>
                  No activity yet — process a meeting to see history.
                </span>
              ) : feedItems.map((log, i) => {
                ...
                return (
                  <div key={i} style={{ display: 'flex', gap: '8px' }}>
                    <div style={{
                      ...
                      background: isActive ? indigo : '#D4D2CC',
                      boxShadow: isActive ? '0 0 6px rgba(79,70,229,0.5)' : 'none',
                    }} />
                    <div>
                      ...
                      <span style={{
                        ...
                        color: isActive ? ink : inkMuted, display: 'block', lineHeight: 1.4,
                      }}>
                        {log.msg}
                      </span>
                    </div>
