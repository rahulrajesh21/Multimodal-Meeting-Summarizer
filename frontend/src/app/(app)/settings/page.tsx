...
export default function SettingsPage() {
  ...
  const [chatLlmBackend, setChatLlmBackend] = useState("lmstudio");

  useEffect(() => {
    setChatLlmBackend(localStorage.getItem("chatLlmBackend") || "lmstudio");
  }, []);

  const handleLlmChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const val = e.target.value;
    setChatLlmBackend(val);
    localStorage.setItem("chatLlmBackend", val);
  };

  const handleDeleteMemory = async () => {
    if (
      !confirm(
        "Are you absolutely sure? This will delete all meetings, graphs, embeddings, and uploaded videos.",
      )
    )
      return;
    setIsDeleting(true);
    ...
    try {
      ...
      if (!res.ok)
        throw new Error((await res.text()) || "Failed to delete memory");
      setSystemStatus({
        type: "success",
        text: "All system memory and data has been successfully deleted.",
      });
    } catch (error: any) {
    ...
    <div style={{ padding: "36px 40px", maxWidth: "860px", margin: "0 auto" }}>
      ...
      <section>
        ...
        >
          ...
          >
            Deleting system memory will permanently erase all meetings,
            generated insights, temporal graph data, embeddings, and uploaded
            videos. Participant roles will be preserved.
          </p>
          <button
          ...
          {systemStatus && (
            <div
              style={{
                ...
                fontSize: "13px",
                background:
                  systemStatus.type === "success" ? "#F0FDF4" : "#FEF2F2",
